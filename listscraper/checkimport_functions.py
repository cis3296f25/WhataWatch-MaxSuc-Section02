...
def checkimport_url(url_string):
    ...
    # inside checkimport_url(url_string)
    url_chunks = url_string.split('/')

    try:
        # User lists: /<user>/list/<listname>/
        if len(url_chunks) > 4 and url_chunks[4] == "list":
            type = "list"
        ...
        elif len(url_chunks) > 4 and url_chunks[4] == "watchlist":
            type = "watchlist"
            username = url_chunks[3]
        ...
        elif len(url_chunks) > 4 and url_chunks[4] == "films" and (len(url_chunks) == 6 or url_chunks[5] == ""):
            # e.g. https://letterboxd.com/mscorsese/films/
            type = "films"
            username = url_chunks[3]
            listname = f"{username.lower()}-films"
            check = True

        # Diary: /<user>/films/diary/  or /<user>/diary/
        elif (len(url_chunks) > 5 and url_chunks[4] == "films" and url_chunks[5] == "diary") \
            or (len(url_chunks) > 4 and url_chunks[4] == "diary"):
            type = "diary"
            username = url_chunks[3]
            listname = f"{username.lower()}-diary"
            check = True

        # Generic Letterboxd "films" pages (site-level listing)
